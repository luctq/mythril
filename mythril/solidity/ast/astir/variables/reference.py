...
class ReferenceVariable(ChildNode, Variable):
    def __init__(self, node: "Node", index=None):
        super().__init__()
        if index is None:
            self._index = node.compilation_unit.counter_astir_reference
            node.compilation_unit.counter_astir_reference += 1
        else:
            self._index = index
    ...
    def points_to(self, points_to):
        ...
        from mythril.solidity.ast.astir.utils.utils import is_valid_lvalue
